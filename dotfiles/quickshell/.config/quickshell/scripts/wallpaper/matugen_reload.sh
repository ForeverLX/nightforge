...
QS_JSON="$HOME/.config/quickshell/scripts/quickshell/qs_colors.json"
QS_JSON_TMP="/tmp/qs_colors.json"
...
# Copy flattened colors to /tmp/qs_colors.json for Quickshell bar
if [ -f "$QS_JSON" ]; then
    cp "$QS_JSON" "$QS_JSON_TMP"
fi
