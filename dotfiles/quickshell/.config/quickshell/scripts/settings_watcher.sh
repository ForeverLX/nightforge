...
CONF_DIR="$HOME/.config/niri/config.kdl"
KEYBINDS_CONF="$HOME/.config/niri/keybindings.kdl"
MONITORS_CONF="$HOME/.config/niri/monitors.kdl"
ENV_CONF="$HOME/.config/niri/env.conf"
AUTOSTART_CONF="$HOME/.config/niri/autostart"
...
inotifywait -m -q -e close_write,moved_to --format '%w%f' "$(dirname "$SETTINGS_FILE")" "$(dirname "$ENV_FILE")" | while read -r filepath; do
    ...
    if [[ "$filepath" == "$SETTINGS_FILE" ]]; then
        ...
        if [[ "$NEW_GENERAL" != "$(cat "$CACHE_DIR/general" 2>/dev/null)" ]]; then
            ...
            [ -n "$LANG" ] && [ "$LANG" != "null" ] && niri msg action set-kb-layout "$LANG" 2>/dev/null

            if [ "$GUIDE_STARTUP" == "true" ]; then
                sed -i 's|^#*[[:space:]]*exec-once = ~/.config/quickshell/scripts/qs_manager.sh toggle guide.*|exec-once = ~/.config/quickshell/scripts/qs_manager.sh toggle guide \&|' "$AUTOSTART_CONF"
            elif [ "$GUIDE_STARTUP" == "false" ]; then
                sed -i 's|^exec-once = ~/.config/quickshell/scripts/qs_manager.sh toggle guide.*|# exec-once = ~/.config/quickshell/scripts/qs_manager.sh toggle guide \&|' "$AUTOSTART_CONF"
            fi
...
EOF
            ...
            if [[ "$GUIDE" == "true" ]]; then
                echo "exec-once = ~/.config/quickshell/scripts/qs_manager.sh toggle guide &" >> "$AUTOSTART_CONF"
            else
                echo "# exec-once = ~/.config/quickshell/scripts/qs_manager.sh toggle guide &" >> "$AUTOSTART_CONF"
            fi
