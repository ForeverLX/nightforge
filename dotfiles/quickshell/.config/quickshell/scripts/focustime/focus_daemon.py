...
def get_active_window():
    try:
        ...
        if not output.strip(): return "Desktop", "Desktop"
        data = json.loads(output)
        
        app_cls = data.get('app-id') or ''
...
def is_locked():
    try:
        subprocess.check_output(['pgrep', '-x', 'niri-lock'])
        return True
        ...
        try:
            ...
            for line in proc.stdout:
                ...
                try:
                    ...
                    continue
                if event.get("Windows") is not None or event.get("FocusWindow") is not None:
                    cls, clean_title = get_active_window()
                    if is_locked() or cls == "niri-lock":
