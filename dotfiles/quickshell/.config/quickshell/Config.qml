...
Item {
    ...
    readonly property string qsDir: homeDir + "/.config/quickshell"
    readonly property string qsScriptsDir: qsDir + "/scripts"
    readonly property string cacheDir: homeDir + "/.cache/quickshell"
    readonly property string niriDir: homeDir + "/.config/niri"
    ...
    property bool topbarHelpIcon: false  // DEPRECATED — waybar handles bar
    ...
    function saveAppSettings() {
        ...
        let patchCmd = config.openGuideAtStartup
            ? `sed -i 's|^#*[[:space:]]*exec-once = ~/.config/quickshell/scripts/qs_manager.sh toggle guide.*|exec-once = ~/.config/quickshell/scripts/qs_manager.sh toggle guide \\&|' "${config.hyprDir}/config/autostart.conf"`
            : `sed -i 's|^exec-once = ~/.config/quickshell/scripts/qs_manager.sh toggle guide.*|# exec-once = ~/.config/quickshell/scripts/qs_manager.sh toggle guide \\&|' "${config.hyprDir}/config/autostart.conf"`;
        sh(patchCmd);

        if (config.workspaceCount !== config.initialWorkspaceCount) {
    // REFACTOR: Waybar migration — TopBar IPC calls removed
    ...
    Process {
        id: niriKeybindsReader
        command: ["python3", config.qsScriptsDir + "/keybinds-parser.sh"]
        running: false
        stdout: StdioCollector {
            onStreamFinished: {
                try {
                    ...
                    console.log("Error parsing niri keybinds:", e);
                }
            }
        }
    }
    ...
    Process {
        id: niriStartupReader
        command: ["python3", config.qsScriptsDir + "/startups-parser.sh"]
        running: false
        stdout: StdioCollector {
            onStreamFinished: {
                try {
                    ...
                    console.log("Error parsing startups:", e);
                }
            }
        }
    }
            ...
            onStreamFinished: {
                try {
                    if (this.text && this.text.trim().length > 0 && this.text.trim() !== "{}") {
                        ...
                        // Map Keybinds
                        if (config.rawSettings.keybinds !== undefined && Array.isArray(config.rawSettings.keybinds) && config.rawSettings.keybinds.length > 0) {
                            let tempBinds = [];
                            ...
                            niriKeybindsReader.running = true;
                        }

                        if (config.rawSettings.startup !== undefined && Array.isArray(config.rawSettings.startup) && config.rawSettings.startup.length > 0) {
                            let tempStartup = [];
                            for (let s of config.rawSettings.startup) {
                                tempStartup.push({ command: s.command || "", name: s.name || "", source: s.source || "" });
                            }
                            config.startupData = tempStartup;
                            config.startupLoaded();
                        } else {
                            niriStartupReader.running = true;
                        }
                    } else {
                ...
                if (config.keybindsData.length > 0) config.keybindsLoaded();
                if (config.startupData.length > 0) config.startupLoaded();
