...
Item {
    ...
    Keys.onReturnPressed: {
        if (currentTab === 2) { 
            ...
            Quickshell.execDetached(["bash", Quickshell.env("HOME") + "/.config/quickshell/scripts/qs_manager.sh", "toggle", target]);
            event.accepted = true;
        }
    }
    ...
    Process {
        id: versionReader
        command: ["bash", "-c", "echo 'nightforge-v1.0.0' 2>/dev/null || echo 'Unknown'"]
        running: true
    ...
    Process {
        id: updateChecker
        command: ["bash", "-c", "curl -m 5 -s https://raw.githubusercontent.com/ForeverLX/nightforge/main/install.sh | grep '^DOTS_VERSION=' | cut -d'\"' -f2"]
        running: true
        stdout: StdioCollector {
            onStreamFinished: {
                let out = this.text ? this.text.trim() : "";
    ...
    property string sysTerminal: "Loading..."
    property string sysWM: "Loading..."
    property string sysMemory: "Loading..."
    property string sysShell: "Loading..."
    ...
    Process {
        id: terminalDetect
        command: ["sh", "-c", "echo \"${TERM_PROGRAM:-$(basename ${TERMINAL:-ghostty})}\""]
        running: true
        stdout: StdioCollector {
            onStreamFinished: {
                let t = this.text ? this.text.trim() : "";
                if (t !== "") root.sysTerminal = t;
            }
        }
    }
    ...
    Process {
        id: wmDetect
        command: ["sh", "-c", "echo niri"]
        running: true
        stdout: StdioCollector {
            onStreamFinished: {
                let w = this.text ? this.text.trim() : "";
                if (w !== "") root.sysWM = w;
            }
        }
    }
    ...
    Process {
        id: uptimeDetect
        command: ["sh", "-c", "uptime -p | sed 's/up //' || echo 'unknown'"]
        running: true
        stdout: StdioCollector {
            onStreamFinished: {
                let u = this.text ? this.text.trim() : "";
                if (u !== "") root.sysUptime = u;
            }
        }
    }
    ...
    Process {
        id: memDetect
        command: ["sh", "-c", "free -h | grep Mem | awk '{print $2}' || echo 'unknown'"]
        running: true
        stdout: StdioCollector {
            onStreamFinished: {
                let m = this.text ? this.text.trim() : "";
                if (m !== "") root.sysMemory = m;
            }
        }
    }
    ...
    Process {
        id: shellDetect
        command: ["sh", "-c", "basename \"$SHELL\" || echo 'sh'"]
        running: true
        stdout: StdioCollector {
            onStreamFinished: {
                let s = this.text ? this.text.trim() : "";
                if (s !== "") root.sysShell = s;
            }
        }
    }
    ...
    function updateSysModel() {
        if (systemDataModel.count > 0 && root.sysWM !== "Loading...") {
            systemDataModel.setProperty(0, "pkg", root.sysWM);
        }
        if (systemDataModel.count > 4 && root.sysTerminal !== "Loading...") {
            systemDataModel.setProperty(4, "pkg", root.sysTerminal);
        }
    }
    onSysWMChanged: updateSysModel()
    onSysTerminalChanged: updateSysModel()
    ...
    SequentialAnimation {
        ...
        ScriptAction { 
            script: Quickshell.execDetached(["bash", Quickshell.env("HOME") + "/.config/quickshell/scripts/qs_manager.sh", "close"]) 
        }
    }
            ...
            ColumnLayout {
                            ...
                            Column {
                                ...
                                Rectangle {
                                    ...
                                    MouseArea { 
                                        ...
                                        onClicked: {
                                            if (index === 0) {
                                                Quickshell.execDetached(["bash", Quickshell.env("HOME") + "/.config/quickshell/scripts/qs_manager.sh", "toggle", "settings"]);
                                            } else {
                ...
                Rectangle {
                    ...
                    MouseArea {
                        ...
                        cursorShape: Qt.PointingHandCursor
                        onClicked: {
                            let cmd = "if command -v ghostty >/dev/null 2>&1; then ghostty --hold bash -c 'eval \"$(curl -fsSL https://raw.githubusercontent.com/ForeverLX/nightforge/main/install.sh)\"'; else ${TERM:-xterm} -hold -e bash -c 'eval \"$(curl -fsSL https://raw.githubusercontent.com/ForeverLX/nightforge/main/install.sh)\"'; fi";
        ...
        Item {
            ...
            Item {
                ...
                ListModel {
                    ...
                    ListElement { pkg: "Quickshell"; role: "Shell & Widgets"; icon: "󰣆"; clr: "mauve"; link: "https://git.outfoxxed.me/outfoxxed/quickshell" }
                    ListElement { pkg: "Matugen"; role: "Theme Engine"; icon: "󰏘"; clr: "peach"; link: "https://github.com/InioX/matugen" }
                    ListElement { pkg: "Fuzzel"; role: "App Launcher"; icon: ""; clr: "green"; link: "https://codeberg.org/dnkl/fuzzel" }
                    ListElement { pkg: "Ghostty"; role: "Terminal Emulator"; icon: "󰄛"; clr: "yellow"; link: "https://ghostty.org" }
                    ListElement { pkg: "Notifications"; role: "Overlays & Notifs"; icon: "󰂚"; clr: "pink"; link: "" }
                }
                    ...
                    Rectangle {
                        ...
                        RowLayout {
                            ...
                            Row {
                                ...
                                spacing: root.s(1)
                                Repeater {
                                    model: root.sysUser.split('').map((l, i) => ({ l, c: [root.red, root.peach, root.yellow, root.green, root.sapphire, root.blue, root.mauve, root.pink][i % 8] }))
                                    Text { 
                                        text: modelData.l
                                        font.family: "JetBrains Mono"
                                        font.weight: Font.Black
                        ...
                        MouseArea { 
                            ...
                            hoverEnabled: true
                            cursorShape: Qt.PointingHandCursor
                            onClicked: Quickshell.execDetached(["xdg-open", "https://github.com/ForeverLX/nightforge"]) 
                        }
                    }
                    ...
                    RowLayout {
                        ...
                        Rectangle {
                            ...
                            MouseArea { 
                                ...
                                hoverEnabled: true
                                cursorShape: Qt.PointingHandCursor
                                onClicked: Quickshell.execDetached(["bash", Quickshell.env("HOME") + "/.config/quickshell/scripts/qs_manager.sh", "toggle", modulesDataModel.get(root.selectedModuleIndex).target]) 
                            }
                        }
                    }
                    ...
                    ListView {
                        ...
                        delegate: Rectangle {
                            ...
                            MouseArea { 
                                ...
                                onDoubleClicked: { 
                                    root.selectedModuleIndex = index; 
                                    Quickshell.execDetached(["bash", Quickshell.env("HOME") + "/.config/quickshell/scripts/qs_manager.sh", "toggle", model.target]) 
                                } 
                            }
                        }
                    }
            ...
            Item {
                ...
                ColumnLayout {
                    ...
                    GridLayout {
                        ...
                        Repeater {
                            model: [ 
                                { f: "ghostty-colors.conf", i: "󰄛", c: "yellow" }, 
                                { f: "nvim-colors.lua", i: "", c: "green" }, 
                                { f: "rofi.rasi", i: "", c: "blue" }, 
                                { f: "cava-colors.ini", i: "󰎆", c: "mauve" }, 
                                { f: "gtklock/style.css", i: "󰍃", c: "peach" },
                                { f: "notifications/style.css", i: "󰂚", c: "pink" } 
                            ]
            ...
            Item {
                ...
                RowLayout {
                    ...
                    Repeater {
                        model: [
                            { name: "NightForge", icon: "", color: "blue", url: "https://github.com/ForeverLX/nightforge" }
                        ]
