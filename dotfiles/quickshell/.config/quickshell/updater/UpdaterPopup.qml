...
Item {
    ...
    property string videoUrl: "https://raw.githubusercontent.com/ForeverLX/nightforge/master/update.mp4"
    ...
    Process {
        id: localVerProcess
        running: false
        command: ["bash", "-c", "echo nightforge-v1.0.0 2>/dev/null
    ...
    Process {
        id: remoteVerProcess
        running: false
        command: ["bash", "-c", "curl -m 5 -s https://raw.githubusercontent.com/ForeverLX/nightforge/master/install.sh | grep '^DOTS_VERSION=' | cut -d'\"' -f2"]
        stdout: StdioCollector {
            onStreamFinished: {
                let out = this.text ? this.text.trim() : "";
...
repo = 'ForeverLX/nightforge'

try:
    local = subprocess.check_output("echo nightforge-v1.0.0 2>/dev/null
except:
    local = ''
            ...
            Rectangle {
                ...
                NumberAnimation {
                    ...
                    onFinished: {
                        ...
                        let cmd = "if command -v kitty >/dev/null 2>&1; then kitty --hold bash -c 'eval \"$(curl -fsSL https://raw.githubusercontent.com/ForeverLX/nightforge/master/install.sh)\"'; else ${TERM:-xterm} -hold -e bash -c 'eval \"$(curl -fsSL https://raw.githubusercontent.com/ForeverLX/nightforge/master/install.sh)\"'; fi";
