...
Item {
    ...
    function maxHighlightForTab(tab) {
        ...
        if (tab === 2) return dynamicStartupModel.count - 1;
        return -1;
    }
    ...
    function scrollHighlightedIntoView() {
        ...
        } else if (root.currentTab === 2 && startupLoader.item) {
    ...
    function handleRootEnter(event) {
        ...
        else if (root.currentTab === 2) root.saveAllStartup();
        event.accepted = true;
    }
    ...
    Connections {
        ...
        function onKeybindsLoaded() {
            ...
            for (let k of Config.keybindsData) {
    ...
    function saveAllStartup() {
        ...
        for (let i = 0; i < dynamicStartupModel.count; i++) {
            ...
            let name = (dynamicStartupModel.get(i).name || "").trim();
            let source = (dynamicStartupModel.get(i).source || "").trim();
            if (cmd.length > 0) startupArray.push({ command: cmd, name: name, source: source });
        }
    ...
    Timer {
        ...
        onTriggered: {
            if (targetBox >= 0) {
                ...
                } else if (targetTab === 2 && startupLoader.item) {
...
property var allSettingsCards: [
        ...
        { tab: 0, boxIndex: 1, label: "Help icon",         desc: "Show button in topbar",  icon: "󰒖", color: "blue" },
    ...
    function getMatchingKeybindIndices(query) {
        ...
        try {
            ...
            for (let i = 0; i < dynamicKeybindsModel.count; i++) {
                let item = dynamicKeybindsModel.get(i);
                if (re.test(item.mods) || re.test(item.key) || re.test(item.dispatcher) || re.test(item.command) || re.test(item.type) || re.test(item.action) || re.test(item.section)) {
                    results.push(i);
                }
            }
        } catch(e) {
                            ...
                            ColumnLayout {
                                ...
                                Item {
                                    ...
                                    Rectangle {
                                        ...
                                        width: sectionText.implicitWidth + root.s(10); height: root.s(18); radius: root.s(4)
                                        anchors.right: editButtonSlide.left; anchors.rightMargin: root.s(6)
                                        anchors.verticalCenter: parent.verticalCenter
                                        color: Qt.alpha(root.blue, 0.12)
                                        border.color: Qt.alpha(root.blue, 0.3); border.width: 1
                                        Text {
                                            id: sectionText; anchors.centerIn: parent; text: model.section
                                    ...
                                    Item {
                                        ...
                                        Item {
                                            ...
                                            Row {
                                                ...
                                                anchors.right: cmdClipRect.shouldMarquee ? undefined : parent.right
                                                Text {
                                                    id: cmdTextMain; text: model.action ? model.action : (model.dispatcher + " " + model.command).trim()
        ...
        Item {
            ...
            ColumnLayout {
                ...
                RowLayout {
                    ...
                    Rectangle {
                        id: headerSaveBtn
                        visible: root.currentTab !== 1 && root.currentTab !== 2 && !root.isSearchMode
                ...
                Item {
                    ...
                    Flickable {
                        ...
                        Rectangle {
                            ...
                            property color c1: root.peach
                            property color c2: root.green
                            ...
                            ColumnLayout {
                                ...
                                Item {
                                    ...
                                    Item {
                                        ...
                                        RowLayout {
                                            anchors.fill: parent; spacing: root.s(6)
                                            Rectangle {
                                                visible: model.source !== ""
                                                width: srcBadgeText.implicitWidth + root.s(6); height: root.s(16); radius: root.s(3)
                                                color: model.source === "niri" ? Qt.alpha(root.mauve, 0.15) : (model.source === "systemd" ? Qt.alpha(root.blue, 0.15) : Qt.alpha(root.teal, 0.15))
                                                border.color: model.source === "niri" ? Qt.alpha(root.mauve, 0.4) : (model.source === "systemd" ? Qt.alpha(root.blue, 0.4) : Qt.alpha(root.teal, 0.4))
                                                border.width: 1
                                                Text {
                                                    id: srcBadgeText; anchors.centerIn: parent
                                                    text: model.source; font.family: "JetBrains Mono"; font.pixelSize: root.s(7)
                                                    color: model.source === "niri" ? root.mauve : (model.source === "systemd" ? root.blue : root.teal)
                                                }
                                            }
                                            Text {
                                                Layout.fillWidth: true
                                                text: {
                                                    ...
                                                    if (model.command !== "") return model.command;
                                                    return "(empty)";
                                                }
                                                font.family: "JetBrains Mono"; font.pixelSize: root.s(10)
                                                color: (model.name !== "" || model.command !== "") ? root.text : root.overlay0
