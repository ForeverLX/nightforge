...
PanelWindow {
    ...
    function performQrScan() {
        ...
        let cmd = `bash ~/.config/quickshell/scripts/screenshot.sh --geometry "${root.geometryString}" --scan-qr`
    ...
    function executeCapture(openEditor, isRecord) {
        let cmd = `bash ~/.config/quickshell/scripts/screenshot.sh --geometry "${root.geometryString}"`
        if (isRecord) {
            cmd += " --record"
