...
Item {
    ...
    property bool isStartup: localFolderModel.count === 0
    property bool isReady: visible && localFolderModel.count > 0
    property bool isSearchActive: window.currentFilter === "Search" && window.hasSearched && searchFolderModel.count === 0
    ...
    Process {
        id: monitorProc
        command: ["sh", "-c", "export PATH=$PATH:/usr/bin:/usr/local/bin:/run/current-system/sw/bin && niri msg --json outputs 2>/dev/null | jq '[.[] | {name: .name}]' 2>/dev/null || echo '[]'"]
        running: false
    ...
    function loadMonitors() {
        //console.log("[MonitorSync] Loading niri outputs...");
    ...
    function applyWallpaper(safeFileName, isVideo) {
        ...
        const logFile = "/tmp/qs_wallpaper_debug.log";
        ...
        if (window.currentFilter === "Search" && window.hasSearched) {
            ...
            if (alreadyExists) {
                const applyScript = `
                    ...
                    echo "$DEST_FILE" > "$HOME/.cache/current_wallpaper"
        ...
        const fullScript = `
            ...
            ${wallpaperCmd}
            echo "${escOriginal}" > "$HOME/.cache/current_wallpaper"
    ...
    function isDownloaded(name) {
        ...
        for (let i = 0; i < srcModel.count; i++) {
            if (srcModel.get(i) ? srcModel.get(i).fileName : "" === name) return true;
        }
        return false;
    }
    ...
    function trySearchFocus() {
        ...
        if (true && searchProxyModel.count === searchFolderModel.count) {
             window.searchIndexRestored = true;
        }
    }
    ...
    Process {
        id: markerScanner
        command: ["bash", "-c", "ls \"" + Quickshell.env("HOME") + "/.cache/wallpaper_picker/colors_markers\" 2>/dev/null | grep '_HEX_' || true"]
        running: true
        stdout: SplitParser {
            onRead: function(data) {
                if (!data || data.trim() === "") return;
                markerModel.append({ fileName: data.trim() });
    ...
    Process {
        id: srcScanner
        command: ["bash", "-c", "find \"" + Quickshell.env("HOME") + "/Pictures/wallpapers\" -maxdepth 1 -type f \\( -iname '*.jpg' -o -iname '*.jpeg' -o -iname '*.png' -o -iname '*.webp' -o -iname '*.gif' -o -iname '*.mp4' -o -iname '*.mkv' -o -iname '*.mov' -o -iname '*.webm' \\) | sort"]
        running: true
        stdout: SplitParser {
            onRead: function(data) {
                if (!data || data.trim() === "") return;
                ...
                srcModel.append({ fileName: base, filePath: fname });
            }
        }
        onExited: {
            window.processMarkers();
    ...
    function processMarkers() {
        ...
        for (let i = 0; i < markerModel.count; i++) {
            let markerName = markerModel.get(i) ? markerModel.get(i).fileName : "" || "";
    ...
    ListModel { id: localFolderModel }

    Process {
        id: localScanner
        command: ["bash", "-c", "ls \"" + Quickshell.env("HOME") + "/.cache/wallpaper_picker/thumbs\" 2>/dev/null | grep -iE '\\.(jpg|jpeg|png|webp|gif|mp4|mkv|mov|webm)$' | sort || true"]
        running: true
        stdout: SplitParser {
            onRead: function(data) {
                if (!data || data.trim() === "") return;
                localFolderModel.append({ fileName: data.trim() });
            }
        }
        onExited: window.syncLocalModel()
    }

    ListModel { id: searchFolderModel }

    Process {
        id: searchScanner
        command: ["bash", "-c", "ls \"" + Quickshell.env("HOME") + "/.cache/wallpaper_picker/search_thumbs\" 2>/dev/null | grep -iE '\\.(jpg|jpeg|png|webp|gif|mp4|mkv|mov|webm)$' | sort || true"]
        running: true
        stdout: SplitParser {
            onRead: function(data) {
                if (!data || data.trim() === "") return;
                searchFolderModel.append({ fileName: data.trim() });
            }
        }
        onExited: window.syncSearchModel()
    }
    ...
    function syncLocalModel() {
        ...
        for (let i = startIdx; i < endIdx; i++) {
            let item = localFolderModel.get(i);
            let fn = item ? item.fileName : undefined;
            if (fn !== undefined) {
                batch.push({ "fileName": fn, "fileUrl": "file://" + Quickshell.env("HOME") + "/.cache/wallpaper_picker/thumbs/" + fn });
            }
        }
        if (batch.length > 0) {
    ...
    function syncSearchModel() {
        ...
        let batch = [];
        for (let i = startIdx; i < endIdx; i++) {
            let item = searchFolderModel.get(i);
            let fn = item ? item.fileName : undefined;
            if (fn !== undefined) {
                batch.push({ "fileName": fn, "fileUrl": "" });
            }
        }
        if (batch.length > 0) {
