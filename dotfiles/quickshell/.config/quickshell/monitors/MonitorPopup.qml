...
Item {
    ...
    Process {
        id: displayPoller
        command: ["sh", "-c", "niri msg --json outputs 2>/dev/null | jq '[.[] | {name: .name, width: .current_mode.width, height: .current_mode.height, refresh: .current_mode.refresh_rate, x: .logical_geometry.x, y: .logical_geometry.y, is_current: .is_focused}]' 2>/dev/null || echo '[]'"]
        running: true
    ...
    function triggerApply() {
        ...
        if (monitorsModel.count === 1) {
            ...
            Quickshell.execDetached(["sh", "-c", "# Niri: monitor configuration differs; use kanshi or niri config " + monitorStr + " ; " + jsonCmd + " ; " + postReloadCmd]);
            ...
            let fullHyprCmd = "# Niri: no batch command equivalent '" + batchCmds.join(" ; ") + "'";
            let safeJson = JSON.stringify(jsonMonitorsArray).replace(/'/g, "'\\''");
            let jsonCmd = "jq '.monitors = " + safeJson + "' ~/.config/hypr/settings.json > ~/.config/hypr/settings.json.tmp && mv ~/.config/hypr/settings.json.tmp ~/.config/hypr/settings.json";
            let postReloadCmd = "swww kill ; sleep 0.2 ; swww-daemon &";
