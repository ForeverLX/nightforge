...
// Border is disabled — managed by compositor.kdl
NIRI_EOF
...
copy_generated "$TMP_DIR/kitty-colors.conf" "$HOME/.config/kitty/colors.conf" "kitty colors"
if [[ -f "$HOME/.config/kitty/colors.conf" ]] && pgrep -x kitty > /dev/null; then
    killall -SIGUSR1 kitty 2>/dev/null || true
    log_info "Reloaded kitty config"
fi

copy_generated "$TMP_DIR/kitty-tab-colors.conf" "$HOME/.config/kitty/tab-colors.conf" "kitty tab colors"
# --- Core outputs ---
...
# --- Bar ---
copy_generated "$TMP_DIR/waybar.css" "$HOME/.config/waybar/style.css" "waybar style"
if pgrep -x waybar > /dev/null; then
    killall -SIGUSR2 waybar
    log_info "Reloaded waybar"
fi
